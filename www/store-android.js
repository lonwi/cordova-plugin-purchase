...
(function () {
    ...
    /*///*/ store.ERR_CLOUD_SERVICE_NETWORK_CONNECTION_FAILED =
        ERROR_CODES_BASE + 25; // Error code indicating that the device could not connect to the network.
...
(function () {
    ...
    store.Product = function (options) {
        ...
        this.introPriceNumberOfPeriods =
            options.introPriceNumberOfPeriods || null; // legacy
        ...
        this.introPriceSubscriptionPeriod =
            options.introPriceSubscriptionPeriod || null; // legacy
                ...
                if (success) {
                    ...
                    if (
                        ...
                        data.ineligible_for_intro_price.forEach(function (pid) {
                            var p = store.get(pid);
                            if (p && p.group) ineligibleGroups[p.group] = true;
                        });
                        store.products.forEach(function (p) {
                            if (
                                data.ineligible_for_intro_price.indexOf(p.id) >=
                                0
                            ) {
                                store.log.debug(
                                    "verify -> " +
...
(function () {
    ...
    store.register = function (product) {
        if (!product) return;
        if (typeof product.length === "number") registerProducts(product);
        else store.register([product]);
    };
    ...
    function runValidation() {
        ...
        Object.keys(byProduct).forEach(function (productId) {
            ...
            if (!product.additionalData.applicationUsername) {
                product.additionalData.applicationUsername = store.getApplicationUsername(
                    product
                );
            }
            if (!product.additionalData.applicationUsername) {
        ...
        function getPrivacyPolicy() {
            ...
            // default: no tracking
            else return ["analytics", "support", "fraud"];
        }
        ...
        function getDeviceInfo() {
            ...
            var device = {};
            if (isObject(this.device)) device = this.device;
            ...
            if (allowed("fraud")) {
                ...
                var fingerprint = "";
                if (device.serial) fingerprint = "serial:" + device.serial;
                // Hardware serial number
                else if (device.uuid) fingerprint = "uuid:" + device.uuid;
                // Device UUID
                else {
                ...
                if (fingerprint) ret.fingerprint = store.utils.md5(fingerprint);
            }
...
(function () {
    ...
    function createPromise() {
        ...
        function genPromise(eventName) {
            return function (cb) {
                if (events[eventName]) cb();
                else callbacks[eventName].push(cb);
                return this;
            };
        }
...
(function () {
    ...
    store.utils = {
        ...
        /* eslint-enable */ // from underscore.js
        ...
        for (i = 0; i < validProducts.length; ++i) {
            ...
            if (p) {
                ...
                var introPriceNumberOfPeriods = vp.introductoryPriceCycles
                    ...
                    : 0;
                var introPricePeriodUnit = normalizeISOPeriodUnit(
                    introPriceSubscriptionPeriod
                );
                var introPricePeriodCount = normalizeISOPeriodCount(
                    introPriceSubscriptionPeriod
                );
                var introPricePeriod =
                ...
                if (vp.freeTrialPeriod) {
                    ...
                    introPriceSubscriptionPeriod = vp.freeTrialPeriod
                        ? vp.freeTrialPeriod
                        : "";
                    introPricePeriodUnit = normalizeISOPeriodUnit(
                        introPriceSubscriptionPeriod
                    );
                    introPricePeriod = normalizeISOPeriodCount(
                        introPriceSubscriptionPeriod
                    );
            ...
            if (!a.oldPurchaseToken && !a.oldSku) {
                ...
                store.getGroup(product.group).forEach(function (otherProduct) {
                    if (isPurchased(otherProduct)) {
                        ...
                        a.oldPurchaseToken = otherProduct.transaction
                ...
                store.products.forEach(function (otherProduct) {
                    var otherPurchaseToken = otherProduct.transaction
                        ? otherProduct.transaction.purchaseToken
                        : null;
